...
### EXTERNAL_IP - Замена всех IP адресов

Если вы хотите перенаправлять все домены из hosts файлов на один IP адрес (например, на ваш собственный сервер), используйте переменную окружения `EXTERNAL_IP`.

Установите в **переменную окружения** `EXTERNAL_IP` нужный IP адрес (например, `10.20.30.40`)

**Как это работает:**
- Когда `EXTERNAL_IP` задана, все IP адреса из hosts файлов будут заменены на указанный IP
- Когда `EXTERNAL_IP` не задана или пустая, будут использоваться оригинальные IP адреса из hosts файлов
- Скрипт будет выводить в консоль информацию о каждом обработанном домене и используемом IP

**Пример:**

Если в hosts файле содержится:
```
1.2.3.4 example.com
5.6.7.8 domain.org
```

И задано `EXTERNAL_IP=10.20.30.40`, оба домена будут перенаправлены на `10.20.30.40`
...
### FORCE_REWRITE - Принудительная полная перезапись (только для NextDNS)

По умолчанию скрипт работает в инкрементальном режиме для NextDNS:
- Добавляются только новые домены
- Обновляются только домены с изменившимися IP
- Существующие настройки остаются нетронутыми

Если вы хотите принудительно перезаписать все настройки NextDNS, установите **переменную окружения** `FORCE_REWRITE` в значение `true`

**Как это работает:**
- Когда `FORCE_REWRITE=true`, ВСЕ существующие перенаправления и блокировки будут удалены из NextDNS перед добавлением новых
- Когда `FORCE_REWRITE` не задана или имеет другое значение, скрипт работает в инкрементальном режиме

**Когда это может быть полезно:**
- Вы хотите, чтобы NextDNS точно соответствовал вашим hosts файлам
- Вы вносили ручные изменения в NextDNS и хотите всё сбросить
- Вы устраняете проблемы и хотите начать с чистого состояния

**⚠️ Внимание:** Это удалит ВСЕ существующие перенаправления и блокировки из NextDNS, включая добавленные вручную!

### Поддержка множественных профилей NextDNS

Вы можете настроить обновление нескольких профилей NextDNS одновременно. Это полезно, если вы:
- Управляете несколькими аккаунтами NextDNS
- Имеете несколько устройств или локаций с разными профилями NextDNS
- Хотите применить одинаковые блокировки/перенаправления к разным профилям

**Базовая конфигурация:**

Укажите несколько ID профилей в `CLIENT_ID` через запятую:
...
1. **Один API ключ для всех профилей (один аккаунт):**
```bash
CLIENT_ID=abc123,def456,ghi789
AUTH_SECRET=ваш_api_ключ
```
Все профили принадлежат одному аккаунту NextDNS

2. **Разные API ключи для каждого профиля (разные аккаунты):**
```bash
CLIENT_ID=abc123,def456,ghi789
AUTH_SECRET=api_ключ_1,api_ключ_2,api_ключ_3
```
Каждый профиль принадлежит своему аккаунту NextDNS

**Конфигурация EXTERNAL_IP:**

1. **Один IP для всех профилей:**
```bash
CLIENT_ID=abc123,def456,ghi789
AUTH_SECRET=key1,key2,key3
EXTERNAL_IP=10.20.30.40
```
Все профили перенаправляют домены на `10.20.30.40`

2. **Разные IP для каждого профиля:**
```bash
CLIENT_ID=abc123,def456,ghi789
AUTH_SECRET=key1,key2,key3
EXTERNAL_IP=10.20.30.40,10.20.30.41,10.20.30.42
```
- Профиль `abc123` использует API `key1` и перенаправляет на `10.20.30.40`
- Профиль `def456` использует API `key2` и перенаправляет на `10.20.30.41`
- Профиль `ghi789` использует API `key3` и перенаправляет на `10.20.30.42`

3. **Смешанная конфигурация (некоторые с EXTERNAL_IP, некоторые без):**
```bash
CLIENT_ID=abc123,def456,ghi789
AUTH_SECRET=key1,key2,key3
EXTERNAL_IP=10.20.30.40,,10.20.30.42
```
- Профиль `abc123` использует `10.20.30.40`
- Профиль `def456` использует IP из hosts файлов
- Профиль `ghi789` использует `10.20.30.42`

**Как это работает:**
- Скрипт загружает hosts файлы один раз (не для каждого профиля)
- Каждый профиль обрабатывается независимо со своим API ключом
- Если один профиль завершается с ошибкой, остальные продолжают обработку
- В конце выводится сводка: сколько профилей успешно обработано/с ошибками

**Преимущества:**
- Обновление нескольких профилей/аккаунтов одной конфигурацией
- Разные API ключи для разных аккаунтов
- Разные EXTERNAL_IP для каждого профиля
- Атомарная операция для каждого профиля (ошибка в одном не влияет на другие)
